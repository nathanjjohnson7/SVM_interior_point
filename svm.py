...
class SVM:
    ...
    def get_bias(self):
        indices = T.where((self.alphas > 0) & (self.alphas <= self.C))[0]
        ay = self.alphas[indices]*self.y[indices]
        ay = ay.repeat((1, indices.shape[0]))
        x_matmul = T.matmul(self.x[indices],self.x[indices].T)
        bias = T.sum(self.y[indices].squeeze(-1) - T.sum(ay*x_matmul, dim=1))
        bias = bias/indices.shape[0]
        return bias
